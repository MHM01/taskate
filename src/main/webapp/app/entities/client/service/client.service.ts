...
export class ClientService {
  ...
  protected convertDateFromClient(client: IClient): IClient {
    return Object.assign({}, client, {
      subscribedAt: client.subscribedAt?.isValid() ? client.subscribedAt.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((client: IClient) => {
        client.subscribedAt = client.subscribedAt ? dayjs(client.subscribedAt) : undefined;
      });
    }
    return res;
  }
